...
// File: contracts/MNT_Mumbai_3.sol
...
contract MNT is ERC20Capped
{
    ...
    modifier onlyOwner()
    {
        require(msg.sender == contractOwner, "Error-MNT_MUMBAI_3.sol-Ownable");
        _;
    }

    function buyTokensFromContract() external payable
    {
        require(msg.value>0,"Error-MNT_MUMBAI_3.sol-Valueless_Transaction");
        // Mint tokens using _mint from ERC20.sol
        uint256 tokensToMint=msg.value*(10**uint256(decimals()))/zTokenPriceInWei() ;
        
        if (msg.sender!=contractOwner)
        {
            require(balanceOf(msg.sender) + tokensToMint <= capPerAccount * (10**uint256(decimals())), "Error-MNT_MUMBAI_3.sol-Exceeds_Max_Allowed_Balance");
        }
    ...
    function approxEquivalent(uint256 amountInWei) public view returns(uint256)
    {
        uint256 ret=amountInWei/zTokenPriceInWei();
        return ret;
    }
...
// Current contract address (Mumbai testnet): 0xF0401aCeCf919797716117710BBE003D5A46502B